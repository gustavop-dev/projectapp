...
class TestRebaselineClientModuleSelection:
    """When the admin changes the set of admin-default calculator modules in
    the functional_requirements section, a prior client-confirmed selection is
    invalidated and selected_modules is re-baselined to the new admin defaults.
    """

    def _fr_content(self, ai_selected):
        return {
            'index': '7',
            'title': 'Requerimientos Funcionales',
            'intro': 'Detalle.',
            'groups': [
                {'id': 'views', 'icon': '🖥️', 'title': 'Vistas',
                 'description': 'Pantallas.', 'items': []},
            ],
            'additionalModules': [
                {'id': 'ai_module', 'icon': '🤖', 'title': 'IA Avanzada',
                 'description': 'Automatización con IA.', 'items': [],
                 'is_calculator_module': True, 'price_percent': 80,
                 'selected': ai_selected, 'default_selected': False},
                {'id': 'reports_alerts_module', 'icon': '📊', 'title': 'Reportes',
                 'description': 'Reportes operativos.', 'items': [],
                 'is_calculator_module': True, 'price_percent': 20,
                 'selected': True, 'default_selected': True},
            ],
        }

    def _confirmed_proposal(self):
        from content.models import ProposalChangeLog

        prop = BusinessProposal.objects.create(
            title='Rebaseline Test', client_name='Client',
            total_investment=6000000,
            selected_modules=['module-reports_alerts_module'],
        )
        section = ProposalSection.objects.create(
            proposal=prop, section_type='functional_requirements',
            title='FR', order=7, content_json=self._fr_content(ai_selected=False),
        )
        ProposalChangeLog.objects.create(
            proposal=prop, change_type='calc_confirmed', actor_type='client',
            description='{"selected": ["module-reports_alerts_module"]}',
        )
        assert prop.has_confirmed_module_selection is True
        return prop, section

    def test_admin_marking_module_resets_client_confirmation(self, admin_client):
        prop, section = self._confirmed_proposal()
        url = reverse('update-proposal-section', kwargs={'section_id': section.id})

        response = admin_client.patch(
            url, {'content_json': self._fr_content(ai_selected=True)}, format='json'
        )

        assert response.status_code == 200
        prop.refresh_from_db()
        assert prop.has_confirmed_module_selection is False
        assert prop.selected_modules == ['module-ai_module', 'module-reports_alerts_module']

    def test_unrelated_fr_edit_keeps_client_confirmation(self, admin_client):
        prop, section = self._confirmed_proposal()
        url = reverse('update-proposal-section', kwargs={'section_id': section.id})
        edited = self._fr_content(ai_selected=False)
        edited['groups'][0]['description'] = 'Pantallas del sitio (editado).'

        response = admin_client.patch(url, {'content_json': edited}, format='json')

        assert response.status_code == 200
        prop.refresh_from_db()
        assert prop.has_confirmed_module_selection is True
        assert prop.selected_modules == ['module-reports_alerts_module']
