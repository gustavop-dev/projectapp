() => {
  ...
  const add = (code, message, element) => issues.push({ severity: 'error', code, message, file: 'template.html', node: element?.dataset.templateNode || null });
  const visible = (el) => {
  ...
  for (const el of nodes) {
    ...
    for (const node of [...el.childNodes]) {
      ...
      if (!css.color.startsWith('rgb') || ambiguous) add('contrast_unverifiable', 'No se puede certificar el contraste de este texto con filtros, mezcla o colores no RGB.', el);
      else texts.push({ rects, color, opacity, threshold: /^H[1-6]$/.test(el.tagName) && parseFloat(css.fontSize) >= 24 ? 3 : 4.5, node: el.dataset.templateNode });
  ...
  for (const animation of animations) {
    ...
    if (timing.iterations === Infinity && (el.textContent.trim() || el.matches('a,button') || el.querySelector('a,button'))) add('continuous_text', 'No se permite animación continua en textos o botones.', el);
