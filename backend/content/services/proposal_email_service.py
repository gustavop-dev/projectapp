...
class ProposalEmailService:
    ...
    @classmethod
    def _attach_commercial_pdf(cls, email, proposal):
        """Attach the commercial proposal PDF to ``email``.

        Returns True when the PDF was generated and attached; False on any
        failure (which is logged and swallowed so the email still sends).
        """
    ...
    def send_proposal_to_client(cls, proposal):
        ...
        Attaches the commercial PDF generated by ``ProposalPdfService``
        (best-effort: a PDF failure is logged but does not block the send).
