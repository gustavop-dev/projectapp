...
| Web App Diagnostics — Full Module | ✅ Done | `WebAppDiagnostic` + `DiagnosticDocument` models (migration `0090`); `diagnostic_service.py` (`create_diagnostic`, `render_document`, `transition_status`, `visible_documents`, `build_render_context`, `register_view`; `@lru_cache` template loading); `diagnostic_email_service.py` (reuses `ProposalEmailService._log_email` + `_is_unsendable_client_email`); 10 FBV endpoints under `/api/diagnostics/`; `diagnostics.js` + `diagnostics_constants.js` Pinia stores; 5 `WebAppDiagnostic/` components (Editor, Viewer, StatusBadge, PricingForm, RadiographyForm); 4 pages (panel index/create/edit + public `diagnostic/[uuid]/index.vue`); 3 `.md` templates with `## Índice` TOC + `{{var}}` placeholders; 4 email templates; 11 backend tests all green; navigation entry under commercial group |
