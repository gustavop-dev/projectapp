...
- **Web App Diagnostics — JSON-Section Rewrite** (Apr 16, 2026):
  ...
  - **Admin UI** (`frontend/pages/panel/diagnostics/[id]/edit.vue`): tab list expanded from 6 to **10** — `summary`, `pricing`, `radiography`, `sections` (new — form editor loop), `plantillas` (renamed — now renders the raw JSON of all sections with copy/save), `prompt` (new), `activity` (new), `analytics` (new), `emails`, `documents`. Legacy `DiagnosticDocumentEditor` deleted; the debounced save timer map now keys by `sectionId`. `resyncJsonBuffer()` guards against clobbering unsaved JSON edits when the user is actively typing in Plantillas.
