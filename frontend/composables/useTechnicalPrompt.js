...
3c. **Profundidad por item (REGLA DE GRANULARIDAD)** — La cobertura mínima de 3b es el piso, no la meta. Desglosa cada item comercial según su complejidad real: una pantalla o módulo típico necesita **2 a 5 requerimientos** (datos que muestra/captura, estados —vacío, cargando, error—, validaciones, integraciones, permisos/roles); solo un elemento trivial o estático (página legal, footer, badge) puede quedar con 1. Cada requerimiento se redacta como **criterio de aceptación verificable**, no como resumen del item. **Anti-requerimiento-manta:** un requerimiento solo puede enlazar más de 3 items en \`linked_item_ids\` si es genuinamente transversal a todos; en ese caso su \`flowKey\` DEBE empezar con \`cross-\` (ej. \`cross-responsive-adaptativo\`). Si no es transversal, repártelo en requerimientos específicos con \`linked_item_ids\` acotados (1-2 ids). **Módulos contratados de \`additionalModules\`:** si escribes la épica de un módulo contratado, cubre cada item del módulo con este mismo estándar (adapta y profundiza — nunca un único requerimiento genérico para todo el módulo); si dejas un módulo contratado sin épica, el backend sembrará su catálogo técnico por defecto (un requerimiento específico por item del módulo, enlazado item por item).
...
5b. **Profundidad:** ≥60% de los items exigibles quedaron con 2 o más requerimientos enlazados; ningún requerimiento no transversal enlaza más de 3 items; los transversales usan \`flowKey\` \`cross-*\`.
