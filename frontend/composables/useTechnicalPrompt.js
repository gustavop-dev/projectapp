...
3c. **Profundidad por item (REGLA DE GRANULARIDAD)** — La cobertura mínima de 3b es el piso, no la meta. El número de requerimientos de cada item sale de su complejidad REAL, nunca de una cuota fija: **4 a 6** si es transaccional o integra servicios (checkout, registro, pasarela, facturación, gestión con roles); **3 a 5** para una pantalla o capacidad típica (dashboard, catálogo, detalle, buscador); **2 a 3** para un elemento informativo simple (página de contenido, tarjeta, banner); **1 a 2** solo para lo trivial o estático (página legal, footer, badge). Dimensiones para desglosar: datos que muestra/captura, estados —vacío, cargando, error—, validaciones y reglas de negocio, integraciones, permisos/roles. **Anti-uniformidad:** los conteos DEBEN variar porque la complejidad varía; si todos los items quedaron con el mismo número de requerimientos, eso es plantilla y no análisis — profundiza los complejos y aligera los triviales. **Anti-plantilla:** PROHIBIDO repetir el mismo patrón de \`title\`/\`flowKey\` de item en item (ej. «Operar X» + «Validaciones y estados de X» para cada X); cada requerimiento nombra datos, estados, reglas o integraciones ESPECÍFICOS de su item — si al reemplazar el nombre del item el texto sirve para cualquier otro, está mal. **Idea general vs. detalle:** la \`description\` del item (paso 1) cuenta el QUÉ y su valor de negocio; cada requerimiento es un **criterio de aceptación verificable**, nunca una paráfrasis del item; el CÓMO técnico vive en \`configuration\` y \`usageFlow\`.**Anti-requerimiento-manta:** un requerimiento solo puede enlazar más de 3 items en \`linked_item_ids\` si es genuinamente transversal a todos; en ese caso su \`flowKey\` DEBE empezar con \`cross-\` (ej. \`cross-responsive-adaptativo\`). Si no es transversal, repártelo en requerimientos específicos con \`linked_item_ids\` acotados (1-2 ids). **Módulos contratados de \`additionalModules\`:** si escribes la épica de un módulo contratado, cubre cada item del módulo con este mismo estándar (adapta y profundiza — nunca un único requerimiento genérico para todo el módulo); si dejas un módulo contratado sin épica, el backend sembrará su catálogo técnico por defecto (1 a 3 requerimientos específicos por item del módulo según su alcance, enlazados item a item).
3d. **Tamaño manejable del requerimiento** — Cada requerimiento cubre un comportamiento o criterio observable que razonablemente cabría en una historia pequeña (**preferencia equivalente a 1–3 puntos**). Si una historia mezcla datos, interacción, integración, recuperación y aceptación y se acercaría a **5 puntos o más**, divídela en requerimientos independientes con \`flowKey\`s propios. Acepta una historia mayor solo cuando separar rompería una regla de negocio indivisible; explica esa indivisibilidad en \`configuration\`.
...
9. **Modelo de datos para integraciones resilientes** — Cuando el producto consume APIs o feeds externos, modela por separado la fuente, el producto/contrato, las ejecuciones de ingestión, los recursos versionados, el estado de salud y la política de retención/cache. Prevé último dato válido, frescura, checksum y expiración cuando aporten continuidad. No conviertas esto en un panel administrativo ni inventes usuarios/roles; es trazabilidad técnica y continuidad de lectura.
...
1. **Una épica por tarjeta contratada:** existe exactamente una épica para \`views\`, \`components\`, \`features\`, cada módulo base y cada módulo adicional contratado — ni una más, ni una menos (salvo una eventual épica transversal al final). Los módulos adicionales contratados son lo único que puedes omitir a propósito: si faltan, el backend siembra su catálogo técnico por defecto.
...
5. **Cobertura total de items:** TODO item comercial (grupos visibles + módulos contratados) aparece en el \`linked_item_ids\` de al menos un requerimiento (para módulos contratados que dejaste sin épica, cuenta la siembra del backend).
5b. **Profundidad y variación:** ≥80% de los items exigibles con 2 o más requerimientos enlazados y los transaccionales/con integraciones con 3 o más; los conteos VARÍAN entre items (todos con el mismo número = plantilla → redistribuir); ningún patrón de \`title\`/\`flowKey\` repetido de item en item; ningún requerimiento no transversal enlaza más de 3 items; los transversales usan \`flowKey\` \`cross-*\`.
5c. **Historias pequeñas:** cada requerimiento valida un comportamiento acotado; todo requerimiento que mezcle varias responsabilidades separables se divide antes de emitir el JSON.
