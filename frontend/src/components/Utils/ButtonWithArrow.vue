<template>
  <div class="flex justify-center">
    <button class="relative flex justify-center items-center py-4 w-3/4 my-4 font-regular rounded-xl bg-esmerald text-white hover-button lg:bg-lemon lg:text-esmerald">
      <span class="inline-block">{{ globalMessages.contact_us }}</span>
 ...
 */
onMounted(() => {
  const button = document.querySelector('.hover-button'); // Select the button element with the class 'hover-button'
  const arrow = button.querySelector('.arrow-icon'); // Select the arrow icon inside the button
  const text = button.querySelector('span'); // Select the text (span) inside the button
