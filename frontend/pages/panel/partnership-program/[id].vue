...
async function simpleAction(action) {
  ...
  if (action === 'create-second-cycle' && result?.id) {
    await navigateTo(localePath(`/panel/partnership-program/${result.id}`))
  }
}
...
<template>
  <BasePageShell width="content">
    ...
    <template v-else-if="agreement">
      <header class="mb-6">
        <NuxtLink :to="localePath('/panel/partnership-program?tab=agreements')" class="text-sm text-text-brand hover:underline">{{ t('financing.agreement.detail.back') }}</NuxtLink>
