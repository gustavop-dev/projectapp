...
async function createAgreement(payload) {
  ...
  await navigateTo(localePath(`/panel/partnership-program/${result.data.id}`))
}
</script>
...
<template>
  <BasePageShell width="content">
    <header class="mb-6">
      <NuxtLink :to="localePath('/panel/partnership-program?tab=agreements')" class="text-sm text-text-brand hover:underline">{{ t('financing.agreement.newPage.back') }}</NuxtLink>
    ...
    <FinancingAgreementForm
      ...
      @cancel="navigateTo(localePath('/panel/partnership-program?tab=agreements'))"
    />
  </BasePageShell>
</template>
