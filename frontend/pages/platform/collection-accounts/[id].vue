<template>
  <div id="platform-collection-account-detail" class="space-y-6">
    <NuxtLink :to="localePath('/platform/collection-accounts')" class="text-sm text-green-light hover:text-text-default dark:hover:text-white">
      ← Back to list
    </NuxtLink>
    ...
    <template v-else>
      <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold text-text-default">{{ doc.title }}</h1>
        ...
        <div class="flex flex-wrap gap-2">
          <button
            type="button"
            class="rounded-xl border border-border-default px-4 py-2 text-sm"
            @click="download"
          >
            Download PDF
          </button>
      ...
      <div class="grid gap-4 lg:grid-cols-2">
        <div class="rounded-2xl border border-border-default bg-surface p-5">
          ...
          <p class="mt-2 text-lg font-bold text-text-brand">Total {{ doc.currency }} {{ doc.total }}</p>
        </div>
        <div v-if="doc.collection_account" class="rounded-2xl border border-border-default bg-surface p-5">
          <h2 class="text-xs font-semibold uppercase tracking-wider text-green-light">Billing</h2>
          <p class="mt-2 text-sm text-text-default">{{ doc.collection_account.billing_concept || '—' }}</p>
          <p class="mt-1 text-xs text-green-light">{{ doc.collection_account.support_reference }}</p>
        </div>
      </div>

      <div v-if="doc.items?.length" class="rounded-2xl border border-border-default bg-surface">
        <h2 class="border-b border-border-default px-5 py-3 text-xs font-semibold uppercase text-green-light">Line items</h2>
        <table class="w-full text-sm">
          <tbody>
            <tr v-for="it in doc.items" :key="it.id" class="border-b border-border-muted">
              <td class="px-5 py-2 text-text-default">{{ it.description }}</td>
      ...
      <div v-if="authStore.isAdmin && doc.commercial_status === 'draft'" class="rounded-2xl border border-border-default bg-surface p-5">
        <h2 class="text-sm font-semibold text-text-default">Add line items (replaces all)</h2>
        <p class="mt-1 text-xs text-green-light">Submit JSON-like rows: description, unit_price, quantity, line_total optional.</p>
        <textarea
          v-model="itemsJson"
          rows="6"
          class="mt-3 w-full rounded-xl border border-border-default p-3 font-mono text-xs dark:bg-primary-strong dark:text-white"
          placeholder='[{"description":"Service","quantity":1,"unit_price":"100000","discount_amount":"0","tax_amount":"0","line_total":"100000"}]'
        />
        <button
          type="button"
          class="mt-3 rounded-xl bg-primary px-4 py-2 text-sm text-white dark:bg-accent dark:text-text-default"
          :disabled="store.isUpdating"
          @click="saveItems"
        >
          Save items
