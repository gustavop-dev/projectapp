  ...
  <div id="platform-deliverable-detail" class="pb-12">
    ...
    <template v-else-if="detail">
      <div class="mb-6" data-enter>
        ...
        <h1 class="text-xl font-bold text-text-default sm:text-2xl">{{ detail.title }}</h1>
      ...
      <div class="mb-8 grid gap-4 lg:grid-cols-2" data-enter>
        <div class="rounded-2xl border border-border-default bg-surface p-5">
          ...
          <div v-if="detail.has_business_proposal" class="space-y-2">
            <p class="text-sm font-medium text-text-default">{{ detail.proposal_title }}</p>
          ...
          <div v-if="detail.file_url" class="mt-4 border-t border-border-default pt-4">
            <p class="mb-2 text-xs font-medium text-green-light/60">Archivo principal</p>
            <a
              :href="detail.file_url"
              target="_blank"
              rel="noopener"
              class="inline-flex items-center gap-2 text-sm font-medium text-text-brand"
            >
              {{ detail.file_name || 'Descargar' }}
            </a>
          </div>

          <div v-if="detail.attachment_files?.length" class="mt-4 border-t border-border-default pt-4">
          ...
          <form v-if="authStore.isAdmin" class="mt-4 border-t border-border-default pt-4" @submit.prevent="uploadAttachment">
            <p class="mb-2 text-xs font-medium text-green-light/60">Subir anexo (admin)</p>
            <div class="flex flex-col gap-2 sm:flex-row sm:items-end">
              <input v-model="attachTitle" type="text" placeholder="Título opcional" class="rounded-xl border border-border-default bg-surface-muted/30 px-3 py-2 text-sm dark:bg-primary-strong dark:text-white" />
              <select v-model="attachCategory" class="rounded-xl border border-border-default bg-surface-muted/30 px-3 py-2 text-sm dark:bg-primary-strong dark:text-white">
              ...
              <button type="submit" :disabled="!attachFile || attachUploading" class="rounded-xl bg-accent px-4 py-2 text-xs font-semibold text-text-default disabled:opacity-50">
          ...
          <div class="mt-4 border-t border-border-default pt-4">
            ...
            <form class="flex flex-col gap-2" @submit.prevent="createClientFolder">
              <div class="flex flex-wrap gap-2">
                <input
                  ...
                  class="min-w-[10rem] flex-1 rounded-xl border border-border-default bg-surface-muted/30 px-3 py-2 text-sm dark:bg-primary-strong dark:text-white"
                />
                <button type="submit" class="rounded-xl border border-border-default px-3 py-2 text-xs font-medium text-text-brand">
                  Crear carpeta
                </button>
              </div>
            </form>
            <form class="mt-3 flex flex-col gap-2 sm:flex-row sm:items-end" @submit.prevent="uploadClientPdf">
              <select
                ...
                class="rounded-xl border border-border-default bg-surface-muted/30 px-3 py-2 text-sm dark:bg-primary-strong dark:text-white"
              >
                <option value="">Sin carpeta</option>
                <option v-for="f in detail.client_folders" :key="f.id" :value="String(f.id)">{{ f.name }}</option>
              </select>
              <input v-model="clientPdfTitle" type="text" placeholder="Título opcional" class="rounded-xl border border-border-default bg-surface-muted/30 px-3 py-2 text-sm dark:bg-primary-strong dark:text-white" />
              <input ref="clientPdfInput" type="file" accept=".pdf,application/pdf" class="text-xs text-green-light" @change="onClientPdfFile" />
              <button type="submit" :disabled="!clientPdfFile || clientPdfUploading" class="rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-white disabled:opacity-50 dark:bg-accent dark:text-text-default">
                {{ clientPdfUploading ? '…' : 'Subir PDF' }}
              </button>
            </form>
          </div>

          <div v-if="detail.collection_accounts?.length" class="mt-4 border-t border-border-default pt-4">
            ...
            <ul class="space-y-2">
              <li v-for="ca in detail.collection_accounts" :key="ca.id" class="flex flex-wrap items-center gap-2 text-sm">
                <span class="font-medium text-text-default">{{ ca.title }}</span>
        ...
        <div class="rounded-2xl border border-border-default bg-surface p-5">
          ...
          <NuxtLink
            ...
            class="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white dark:bg-accent dark:text-text-default"
          >
            Abrir tablero
      ...
      >
        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div
            ...
            :key="entity.id"
            class="rounded-xl border border-border-default p-4"
          >
            <h3 class="text-sm font-semibold text-text-default">{{ entity.name }}</h3>
