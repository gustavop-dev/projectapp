  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-50">
    <div class="bg-surface rounded-xl shadow-sm border border-border-default p-8 max-w-md w-full text-center">
      <div v-if="isLoading" class="space-y-4">
        ...
        <p class="text-sm text-text-muted">Conectando con LinkedIn...</p>
      </div>
      <div v-else-if="success" class="space-y-4">
        <div class="w-12 h-12 bg-primary-soft rounded-full flex items-center justify-center mx-auto">
          <svg class="w-6 h-6 text-text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
        </div>
        <p class="text-sm text-text-default font-medium">LinkedIn conectado correctamente</p>
        <p class="text-xs text-text-muted">Puedes cerrar esta ventana.</p>
      </div>
