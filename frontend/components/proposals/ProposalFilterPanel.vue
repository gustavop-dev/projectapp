  ...
  <div v-show="isOpen" class="mb-4">
    <div class="bg-surface border border-border-default rounded-xl divide-y divide-border-muted">
      ...
      <div class="flex flex-wrap items-center gap-2 px-3 py-2.5">
        <span class="text-[10px] font-semibold uppercase tracking-wider text-text-muted w-[5.5rem] shrink-0">Clasificación</span>
      ...
      <!-- Valores -->
      <div class="flex flex-wrap items-center gap-2 px-3 py-2.5">
        <span class="text-[10px] font-semibold uppercase tracking-wider text-text-muted w-[5.5rem] shrink-0">Valores</span>
        <ProposalFilterDropdown
          label="Moneda"
      ...
      <!-- Fechas -->
      <div class="flex flex-wrap items-center gap-2 px-3 py-2.5">
        <span class="text-[10px] font-semibold uppercase tracking-wider text-text-muted w-[5.5rem] shrink-0">Fechas</span>
        <ProposalFilterRangeDropdown
          label="Creación"
          type="date"
      ...
      <!-- Otros + Limpiar todo -->
      <div class="flex flex-wrap items-center gap-2 px-3 py-2.5">
        <span class="text-[10px] font-semibold uppercase tracking-wider text-text-muted w-[5.5rem] shrink-0">Otros</span>
        <ProposalFilterDropdown
          label="Activo"
        ...
        <div ref="engagementRef" class="relative">
          ...
          <Transition name="dropdown-fade">
            <div
              v-if="engagementOpen"
              class="absolute top-full left-0 mt-1 z-50 w-60 bg-surface border border-border-default rounded-xl shadow-raised py-2"
            >
              <label class="flex items-center gap-2.5 px-3 py-2 text-sm text-text-default hover:bg-surface-raised cursor-pointer">
                <input
                  type="checkbox"
                  data-testid="filter-panel-technical-viewed"
                  :checked="modelValue.technicalViewed"
                  class="w-3.5 h-3.5 rounded border-input-border accent-teal-600"
    ...
    <div v-if="activeChips.length > 0" class="flex flex-wrap items-center gap-1.5 mt-2 px-1">
      <span class="text-[10px] font-semibold uppercase tracking-wider text-text-muted mr-0.5">Activos:</span>
      <span
