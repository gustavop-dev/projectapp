<template>
  <div v-if="issues.length" class="mt-4 bg-warning-soft border border-warning-strong/30 rounded-xl p-4">
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <h4 class="text-sm font-semibold text-warning-strong mb-1">JSON con formato desactualizado</h4>
        <p class="text-xs text-warning-strong mb-3 leading-relaxed">
        ...
        <ul class="text-xs text-warning-strong space-y-1 mb-4 pl-4 list-disc">
          <li v-for="issue in issues" :key="issue">
            <code class="font-mono bg-warning-strong/15 px-1 rounded text-[11px]">{{ fieldLabels[issue] || issue }}</code>
          </li>
        </ul>
        <p class="text-xs text-warning-strong mb-3">{{ actionLabel }}</p>
        <button
          type="button"
