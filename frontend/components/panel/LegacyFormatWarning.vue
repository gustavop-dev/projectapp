<template>
  <div v-if="issues.length" class="mt-4 bg-amber-50 border border-amber-300 rounded-xl p-4">
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <h4 class="text-sm font-semibold text-amber-800 mb-1">JSON con formato desactualizado</h4>
        ...
        <ul class="text-xs text-amber-700 space-y-1 mb-4 pl-4 list-disc">
          <li v-for="issue in issues" :key="issue">
            <code class="font-mono bg-amber-100 px-1 rounded text-[11px]">{{ fieldLabels[issue] || issue }}</code>
          </li>
        </ul>
        <p class="text-xs text-amber-700 mb-3">{{ actionLabel }}</p>
        <button
          type="button"
