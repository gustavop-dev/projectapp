...
import { formatStateDuration, relativeStateTime, stateBadgeVariant } from '~/utils/documentState';
...
async function correctOpening(episode) {
  ...
  const result = await stateStore.correctEpisode(
    props.documentId, episode.id, parsed.toISOString(),
  );
  if (result.success) {
...
<template>
  <BaseModal :model-value="modelValue" kind="detail" padding="none" @update:model-value="emit('update:modelValue', $event)">
    ...
    <div class="max-h-[calc(100dvh-5rem)] space-y-3 overflow-y-auto p-4 sm:max-h-[75vh] sm:p-6" data-testid="document-state-history">
      ...
      <p v-else-if="!stateStore.history.length" class="py-8 text-center text-sm text-text-muted">Todavía no hay episodios.</p>
      <template v-else>
      <article
        v-for="episode in stateStore.history"
        :key="episode.id"
      ...
      >
        ...
        <div class="mt-3 flex justify-end">
          <BaseButton variant="ghost" size="sm" @click="correctOpening(episode)">Corregir apertura</BaseButton>
        </div>
      </article>
      </template>
    </div>
