<template>
  <section ref="sectionRef" class="final-note py-16 md:py-24 bg-surface-muted">
    <div class="container mx-auto px-6 md:px-12 lg:px-24 max-w-4xl">
      ...
      <div data-animate="fade-up" class="note-content bg-surface p-5 sm:p-8 md:p-12 rounded-2xl shadow-sm mb-8">
        ...
        <div class="message-text space-y-6 text-lg text-text-default leading-relaxed">
          ...
          <p v-if="personalNote" class="italic text-text-muted">
            {{ personalNote }}
          </p>
        </div>

        <div class="signature mt-12 pt-8 border-t border-border-default">
      ...
      <div data-animate="fade-up-stagger" class="commitment-badges grid md:grid-cols-3 gap-6">
        <div v-for="(badge, index) in commitmentBadges" :key="index"
             class="badge-card bg-surface p-6 rounded-xl shadow-sm text-center hover:shadow-md transition-shadow">
          <div class="text-4xl mb-3">{{ badge.icon }}</div>
          <h4 class="font-bold text-text-default mb-2">{{ badge.title }}</h4>
          <p class="text-sm text-text-muted">{{ badge.description }}</p>
        </div>
      </div>
      ...
      <div v-if="kickoffPlan && kickoffPlan.length" data-animate="fade-up" class="mt-16 mb-12">
        <h3 class="text-2xl font-bold text-text-brand mb-2 text-center">{{ kickoffTitle }}</h3>
        <p class="text-sm text-text-muted text-center mb-8">{{ kickoffSubtitle }}</p>
        <div class="relative">
          <div class="absolute left-6 sm:left-8 top-0 bottom-0 w-0.5 bg-primary-soft"></div>
          <div v-for="(step, kIdx) in kickoffPlan" :key="kIdx" class="relative flex items-start gap-4 sm:gap-6 mb-6 last:mb-0">
            <div class="relative z-10 flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 bg-primary text-accent rounded-full flex items-center justify-center font-bold text-sm sm:text-base shadow-md">
              {{ step.day || `D${kIdx + 1}` }}
            </div>
      ...
      <template v-if="nextSteps && nextSteps.length">
        ...
        <div data-animate="fade-up-stagger" class="grid md:grid-cols-3 gap-4 sm:gap-8 mb-12">
          <div v-for="(step, sIdx) in nextSteps" :key="sIdx"
               class="step-card bg-primary-soft p-5 sm:p-8 rounded-2xl border-2 border-border-default hover:border-border-muted transition-all text-center">
            ...
            <p class="text-text-muted font-light leading-relaxed">{{ step.description }}</p>
          </div>
        </div>

        <div v-if="ctaMessage" data-animate="fade-up" class="bg-primary p-5 sm:p-8 md:p-12 rounded-3xl text-accent text-center mb-12 shadow-2xl">
          <h3 class="text-2xl sm:text-3xl md:text-4xl font-bold text-accent mb-4">¿Listo para Comenzar?</h3>
          <p class="text-base sm:text-xl text-accent mb-6 sm:mb-8 max-w-2xl mx-auto">
            {{ ctaMessage }}
          </p>
          <div class="cta-buttons flex flex-col md:flex-row gap-4 justify-center">
            <a v-if="primaryCTA?.link" :href="primaryCTA.link" target="_blank"
               class="inline-flex items-center justify-center px-6 sm:px-8 py-3 sm:py-4 bg-accent text-text-brand rounded-xl font-bold text-base sm:text-lg hover:opacity-90 transition-all shadow-lg hover:shadow-xl">
            ...
            <button v-if="secondaryCTA?.text" type="button"
               ...
               class="inline-flex items-center justify-center px-6 sm:px-8 py-3 sm:py-4 bg-primary-strong text-accent rounded-xl font-bold text-base sm:text-lg hover:opacity-80 transition-all cursor-pointer">
              <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="contactMethods?.length" data-animate="fade-up-stagger" class="grid md:grid-cols-3 gap-6 mb-12">
          <div v-for="(contact, cIdx) in contactMethods" :key="cIdx"
               class="contact-card bg-primary-soft p-4 sm:p-6 rounded-xl text-center border border-border-default hover:border-border-muted transition-colors">
            <div class="text-4xl mb-3">{{ contact.icon }}</div>
            <h4 class="font-bold text-text-brand mb-2">{{ contact.title }}</h4>
            <a :href="contact.link" target="_blank" class="text-text-muted hover:text-text-brand font-medium transition-colors">
              {{ contact.value }}
            </a>
          </div>
        </div>
