...
| `BaseButton`    | `variant` (`primary`/`secondary`/`ghost`/`danger`/`danger-ghost`/`link`/`accent`), `size` (`sm`/`md`/`lg`), `textPolicy` (`atomic` default / `wrap`), `loading`, `disabled`, `disabledReason`, `title`, `nativeTitle`, `iconOnly`, `as` — see [Button variants](#button-variants) |
...
| `BaseActionButton` | `action`, `label`, `tooltip`, `statusLabel`, `variant`, `size`, `loading`, `disabled`, `disabledReason`, `as`, `to` — canonical icon-only action with one viewport-aware hover/focus tooltip, no native duplicate, and an accessible name |
...
| `BaseOverflowText` | `text`, `to`, `lines` (1/2), `stretch`, `expandable`, `testId`, `contentClasses`; measures real clipping, adds one floating `BaseTooltip` only on overflow and exposes an in-place touch disclosure |
...
Use `BaseActionButton` for an icon-only control. It supplies one app-owned
hover/focus tooltip and an accessible name without also publishing a browser
`title`; `statusLabel` announces transient feedback without replacing the
action glyph. Text buttons and menu items may
