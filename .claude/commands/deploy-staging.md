...
```bash
cd /home/ryzepeck/webapps/projectapp_staging && git status
```
...
```bash
cd /home/ryzepeck/webapps/projectapp_staging && git fetch origin && git branch -r | grep $ARGUMENTS
```
...
```bash
cd /home/ryzepeck/webapps/projectapp_staging && git fetch origin && git checkout $ARGUMENTS && git pull origin $ARGUMENTS
```

5. Install backend dependencies and run migrations:
```bash
cd /home/ryzepeck/webapps/projectapp_staging/backend && source venv/bin/activate && pip install -r requirements.txt && python manage.py migrate
```

6. Build the frontend (requires nvm for Node 22.13.0) and remove node_modules afterwards:
```bash
bash -c 'export NVM_DIR="$HOME/.nvm"; source "$NVM_DIR/nvm.sh"; nvm use; cd /home/ryzepeck/webapps/projectapp_staging/frontend && npm ci && npm run build && rm -rf node_modules'
```
...
```bash
ls /home/ryzepeck/webapps/projectapp_staging/frontend/node_modules 2>/dev/null && echo "WARNING: node_modules still present" || echo "OK: node_modules removed"
```

7. Collect static files:
```bash
cd /home/ryzepeck/webapps/projectapp_staging/backend && source venv/bin/activate && python manage.py collectstatic --noinput
```
...
```bash
sudo systemctl restart projectapp_staging && sudo systemctl restart projectapp-staging-huey
```
...
```bash
sudo systemctl is-active projectapp_staging && sudo systemctl is-active projectapp-staging-huey
```
...
```bash
cd /home/ryzepeck/webapps/projectapp_staging && git log --oneline -1
```
...
```bash
sudo journalctl -u projectapp_staging --no-pager -n 50
```

13. Check Huey logs:
```bash
sudo journalctl -u projectapp-staging-huey --no-pager -n 50
```
...
```bash
tail -50 /home/ryzepeck/webapps/projectapp_staging/backend/debug.log
```

16. If services won't start, check systemd details:
```bash
sudo systemctl status projectapp_staging --no-pager -l
sudo systemctl status projectapp-staging-huey --no-pager -l
```
