---
name: plan-task
description: "Wrapper parametrizado de `/plan` para ProjectApp — recibe en `$ARGUMENTS` la descripción explícita de la tarea/feature/refactor/bug a planear y produce un plan decision-complete sin volver a preguntar el alcance al usuario."
...
## Goal
Aplicar el workflow `/plan` de ProjectApp sobre una tarea cuya descripción ya viene completa en los argumentos. Producir un plan que otro agente o ingeniero pueda implementar sin tomar nuevas decisiones de producto o arquitectura, **sin pedir al usuario que repita el alcance** que ya entregó como parámetro.
...
- Cumplir las reglas del repo: respetar `AGENTS.md`, `backend/AGENTS.md`, `frontend/AGENTS.md`, `CLAUDE.md`, y la guía en `docs/methodology/`.
...
2. **Inspección del repo.** Buscar evidencia: archivos referenciados en `$ARGUMENTS`, patrones existentes (stores Pinia, FBV de DRF, composables), tests relacionados. Usar Explore agents en paralelo si el alcance toca >2 áreas.
3. **Mapeo del cambio.** Data flow afectado, APIs, estado de Pinia, superficie de tests, migraciones.
...
- Reusar utilidades, composables y servicios existentes antes de proponer código nuevo (regla de oro de ProjectApp).
- No proponer rewrites de stores Pinia hacia setup-style; mantener Options API si así está hoy.
- Preservar separación `panel` (sesión Django) vs `platform` (JWT) y los dos clientes HTTP (`request_http.js` vs `usePlatformApi.js`).
- No proponer cambios a migraciones existentes; sí proponer migraciones nuevas si el plan lo requiere.

## Ejemplos de invocación
- `/plan-task agregar paginación server-side al listado de propuestas en /panel/proposals (20 por página, mantener filtros actuales)`
- `/plan-task refactor: extraer la lógica de cálculo de totales del componente CostSection.vue a un composable reutilizable, sin cambiar la UI`
