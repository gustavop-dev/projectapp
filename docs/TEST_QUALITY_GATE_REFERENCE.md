...
- Frontend unit tests (`Jest` / `Vitest` — framework-agnostic, covers Vue and React)
...
- `run`: execute both engines and merge findings:
  - Ruff on backend test targets (curated PT rules)
...
| `--backend-app` | string | `candle_app` | Django app name for backend tests |
...
# Scoped backend file
python3 scripts/test_quality_gate.py --repo-root . --suite backend \
  --include-file backend/candle_app/tests/models/test_dynamic_document.py
```
