...
- **View Map operational Explorer** (`/panel/views`) — a third presentation mode complements the complete route list and module map with a commercial, orbital view of the ProjectApp ecosystem. The first level exposes every product domain; Platform supports deep navigation through eight operational capabilities and their functional relationships. Search, direct links, drag, keyboard rotation, zoom, pause and reduced-motion support make the experience suitable for internal discovery and guided client demonstrations. Route names and source files remain available as secondary disclosure instead of leading the experience.
