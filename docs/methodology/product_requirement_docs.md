...
- **Hierarchical folders & tags**: documents organize into a tree of `DocumentFolder`s (up to **5 levels** deep) plus a flat set of `DocumentTag`s, all managed inline from the documents list page.
  ...
  - **Header "Crear carpeta" shortcut**: the page header exposes a secondary button next to "Nuevo Documento" that opens `FolderManagerModal` directly, with the name input autofocused via `nextTick` — keyboard-only flow works from this entry-point.
  - **Folder creation highlight**: newly-created folders receive `folder-highlight-flash` for 2.5 s in the sidebar (primary-soft background + ring fade) so the user always sees where the folder landed, even in dense trees. Backed by `document_folders.js` store field `newlyCreatedId` + a `setTimeout` that auto-clears it.
  - **Auto-expand on nested create**: if the new folder has ancestors, the modal calls `useFolderExpansion().expandPath(...)` so the highlight is visible even when the parent branch was previously collapsed.
  - **Parent picker is a custom popover (`FolderTreeSelect`)**, not a native `<select>`: indented tree with badges for the depth limit ("Nivel máx."), optional search visible when ≥ 5 folders, dismiss by ESC or click-outside. The previous native select dropped indentation in most browsers and could not surface a depth-limit indicator visually.
  - **Parent pre-selected on create**: when the user opens the create form while browsing inside a folder, the `FolderTreeSelect` trigger pre-fills with that folder's path so the new carpeta lands there without an extra click. Implemented via `:default-parent` prop on `FolderManagerModal` reading `documentStore.activeFolderId` (numeric → that id, `'all'`/`'none'` → null).
  - **Inline trash on every sidebar folder row**: hover over a folder in the sidebar reveals a red trash icon. Click opens a small confirmation modal with two states: destructive (red Eliminar) for empty folders, or amber blocked panel for folders with subcarpetas/descendant documents (shows the backend's specific `detail` message and disables Eliminar). Mirror of the manager modal's deletion UX, surfaced one click closer for power users.
  - **Row actions unified under a kebab + actions sheet on all viewports**: a single 3-dots button per row opens `DocumentActionsSheet` with 9 actions (Editar contenido, Vista previa, Renombrar, Mover a carpeta, Enviar por correo, Descargar PDF, Copiar markdown, Duplicar, Eliminar). Replaces the previous "9 inline icons in md+ / kebab on mobile" split — single source of truth, less visual noise, paridad mobile/desktop. Row body click still navigates to the edit page (kebab uses `@click.stop`).
