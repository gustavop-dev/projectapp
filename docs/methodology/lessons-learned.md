...
### Apply the same reactive class to every surface that renders the entity

Corollary of the above. When `FolderTreeNode.vue` (sidebar) got `folder-highlight-flash` via `:class="{ ... : folder.id === folderStore.newlyCreatedId }"`, we forgot to apply it to `FolderManagerModal.vue` — which renders the same folder list in its own flat markup. Result: creating a folder from inside the modal showed a flash in the sidebar but not in the modal where the user's eyes were. Fix: one extra `:class` binding on the modal row using the **same** store-backed predicate. Lesson: when introducing reactive feedback driven by a store field, grep all components that render the entity (`grep -l "folder\.id\|folder_id" frontend/components`) and audit each for the binding. The store is the source of truth; multiple surfaces can subscribe without coordination.

### One UI surface beats two when responsiveness diverges them

The documents list had two affordances for the same row actions: 9 inline icons on `md+` (`hidden md:flex`) and a kebab + sheet on `<md` (`md:hidden`). They diverged in selectors, in keyboard-accessibility, and in what specs target. When the user asked for "make the modal of actions appear on desktop too", the right answer was **collapse to one** (kebab + sheet everywhere) rather than ship both. Benefits: single E2E selector chain (`kebab → action`), single mental model for the user, less DOM weight, more horizontal space for table data. The cost is that desktop users lose the one-hover-one-click latency for individual actions — acceptable trade-off for the consistency. Test for this anti-pattern: if your responsive breakpoints introduce structurally different DOM (not just style differences), consider whether one of the variants is dead weight.

### Prop-driven default state for modals beats internal heuristics

When `FolderManagerModal.vue` needed to pre-fill the parent picker with the currently-active folder, the temptation was to import `useDocumentStore` inside the modal and read `activeFolderId` directly. We chose the prop path instead: `:default-parent="activeFolderForCreate"` from `index.vue` with a small computed (`typeof id === 'number' ? id : null`). Reasons: (1) the modal stays decoupled from `documentStore` — it only knows about folders. (2) Testing becomes trivial: pass the prop, no store mocking. (3) Future reuse: any other page that opens the modal can pass a different default. The watcher reads `props.defaultParent ?? null` once when the modal opens, after `fetchFolders` resolves so the `FolderTreeSelect` trigger can immediately render the breadcrumb path of the pre-selected folder.

### Sidebar inline actions need a dedicated confirm surface

The manager modal already had a deletion confirm panel with destructive/blocked states using the backend's 409 `detail`. Adding an inline trash button to each sidebar row was tempting to wire directly to `store.deleteFolder(id)` for speed, but that loses the safety net: no confirm for destructive action, no useful message when the backend rejects. The right path was to mirror the manager modal's UX as a small dedicated Teleport in `index.vue`, sharing the backend response shape (`{ detail, reasons, has_children, document_count }`) so both surfaces speak the same language. The user gets faster access without losing the safety net. Keep both surfaces in sync if the backend's response shape evolves.
