...
- **Steps:**
  ...
  4. Either action confirms first: before any target is chosen, the modal already names the selected records; after choosing one it breaks the selection into what gains a client, what changes one and from whom. On open it focuses the filter and immediately renders a permanent A-Z client catalog — no click or typing is required. Typing filters that same visible catalog, the Nombre header toggles A-Z/Z-A and remembers the choice for later openings, later pages load progressively inside the list, and an empty catalog or unmatched filter explains the state and offers inline client creation. At least five complete rows are visible, each with name/company/email and a "Sin correo" warning where applicable; a long result set scrolls inside the catalog without moving the modal. Desktop height follows the content and compact screens use the full viewport. Cancelling writes nothing.
...
- **Coverage:** ✅ Covered (client column + Sin cliente tab, bulk assignment confirming the scope before the payload, permanent client catalog visible without typing, five complete rows with list-only scroll, A-Z/Z-A persistence, explicit unmatched-filter creation, full-screen compact modal, four-row review visible before choosing a target, the disabled-assign guard, the unlink action sending only the linked rows, totals modal breakdown, grouped landing mode dictated by the backend setting, session-only toggle back to classic writing nothing, la selección depurándose tras un borrado —clásica, agrupada y tras "Seleccionar los N filtrados"— y el 409 reconciliando)
